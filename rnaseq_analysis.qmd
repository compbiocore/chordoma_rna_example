...
(e.g. Hallmark gene sets), simply change the `category` / `subcollection` arguments — explore
...
confidence). One entry per gene symbol is kept (highest |stat|). The "stat" column in DESeq2 represents the test statistic for each gene, calculated as the log2 fold change divided by the standard error of the log2 fold change (lfcSE). 
